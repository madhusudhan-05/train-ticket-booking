...
function calculateTotalDistance(train, sourceCode, destinationCode) {
    ...
    for (let i = start; i < end; i++) {
        const segment = train.segments.find(
            (seg) => seg.from === stops[i] && seg.to === stops[i + 1]
        );
...
function checkSeatAvailability(train, sourceCode, destinationCode, seatClass, passengers) {
    const stops = [
        train.source.code,
        ...train.middleStops.map((stop) => stop.code),
        train.destination.code,
    ...
    for (let i = start; i < end; i++) {
        ...
        if (segment.seats[seatClass] < minSeats) {
            minSeats = segment.seats[seatClass];
        }
    }
    return minSeats >= passengers;
}
...
function updateSeats(train, sourceCode, destinationCode, passengers) {
    ...
    for (let i = start; i < end; i++) {
        ...
        passengers.forEach((passenger) => {
            segment.seats[passenger.seatClass] -= 1; 
        });
    }
...
function saveBooking(bookingID, train, sourceCode, destinationCode, passengers, totalFare, totalDistance) {
    const arrivalSegment = train.segments.find(
        (seg) => seg.from === destinationCode || seg.to === destinationCode
    );
    const booking = {
        BookingID: bookingID,
        ...
        Arrival_Time: arrivalSegment.arrivalTime,
...
function BookTicket() {
    ...
    const sourceCode = readLine.question("Enter the source station code: ").toUpperCase();
    const destinationCode = readLine.question("Enter the destination station code: ").toUpperCase();
    ...
    const trainNumber = parseInt(readLine.question("Enter the train number: "));
    ...
    for (let i = 0; i < numPassengers; i++) {
        ...
        while (true) {
            name = readLine.question(`Enter name for Passenger ${i + 1}: `).trim();
            if (name === "") {
        ...
        while (true) {
            age = parseInt(readLine.question("Age: "));
        ...
        while (true) {
            seatClass = readLine.question("Class (GEN, SL, AC): ").toUpperCase().trim();
    ...
    const seatsAvailable = passengers.every((passenger) => {
        return checkSeatAvailability(selectedTrain, sourceCode, destinationCode, passenger.seatClass, numPassengers);
    });
    ...
    const confirm = readLine.question("Confirm booking (yes/no): ").toLowerCase();
    if (confirm !== "yes") {
...
function CancelTicket() {
    ...
    const bookingID = readLine.question("Enter Booking ID: ");
    ...
    const passengerIndices = readLine
        .question("Enter the passenger number to cancel (comma-separated): ")
        .split(",")
        .map(Number)
        .map((i) => i - 1);
